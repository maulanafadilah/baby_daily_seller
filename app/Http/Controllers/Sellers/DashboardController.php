...
class DashboardController extends Controller
{
    public function index()
    {
        ...
        $count = Products::join('sellers', 'sellers.id', '=', 'products.id_penjual')
            ->where('sellers.nomor_telepon', auth()->user()->nomor_telepon)
            ->select('products.id', 'products.nama_produk', 'products.cover', 'products.stok')
            ->latest('products.created_at')->count();
        ...
        return view('sellers/dashboard/dashboard', compact('page_title', 'page_description', 'action', 'header', 'search', 'extraHeader', 'footer', 'bottom', 'sidebar', 'roles', 'sellers', 'products', 'stok', 'count'));
    }
